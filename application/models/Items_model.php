...
class Items_model extends CI_Model {
    ...
    public function _search($inDATA) {
        if (isset($inDATA)) {
            ...
            $this->db->join('`lesson_plans`.`item_images` AS `item_images`', 'item_images.item_id=items.item_id', 'LEFT');
            $this->db->join('`lesson_plans`.`images` AS `item_image`', 'item_image.image_id=item_images.image_id', 'LEFT');
            ...
            $this->db->join('`lesson_plans`.`item_tags` AS `item_tags2`', 'item_tags2.item_id=items.item_id', 'LEFT');
            $this->db->join('`lesson_plans`.`tags`', 'item_tags.tag_id=tags.tag_id', 'LEFT');
            $this->db->join('`lesson_plans`.`tags` AS `tags2`', 'item_tags2.tag_id=tags2.tag_id', 'LEFT');
            ...
            $this->db->group_by('items.item_id,tags.tag_id,files.file_id');
            $this->db->limit(1000);
            $result = $this->db->get();
            $returnDATA=array();
    ...
    public function item_details($inDATA) {
        if (isset($inDATA, $inDATA['item_id'])) {
            ...
            $this->db->where('`items`.`item_id`=' . $inDATA['item_id']);
            if(isset($inDATA['user_id']) === TRUE){
                $this->db->select('reviews.text AS review');
                $this->db->join('lesson_plans.reviews','reviews.item_id = items.item_id AND reviews.created_by = "'.$inDATA['user_id'].'"','LEFT');
            }else{
                $this->db->select('"" AS review');
            }
